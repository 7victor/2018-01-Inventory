...
<body data-type="widgets">
							...
							<div class="widget-body  am-fr">
								...
								<div class="am-u-sm-12 am-u-md-6 am-u-lg-3">
									<div class="am-form-group tpl-table-list-select">
										<select data-am-selected="{btnSize: 'sm'}" id="goodType">
											<option value="0">所有类别</option>
											<c:forEach items="${goodTypeList }" var="goodType">
												<c:if test="${goodVo.goodType==goodType.typeId }">
												<option value="${goodType.typeId }" selected="selected">${goodType.typeName }</option></c:if>
												<c:if test="${goodVo.goodType!=goodType.typeId }">
												<option value="${goodType.typeId }">${goodType.typeName }</option></c:if>
											</c:forEach>
										</select>
									</div>
								</div>
								<div class="am-u-sm-12 am-u-md-12 am-u-lg-3">
									<div
										...
										<input type="text" placeholder="商品编号" class="am-form-field " id="goodId" value="${goodVo.goodId }"> <span
										...
										<tbody>
											<c:forEach items="${goodList.list }" var="good">
												<tr class="gradeX">
													...
													<td>${good.goodNum }</td>
													<td>
													<fmt:formatDate value='${good.goodLastTime }'
														pattern='yyyy-MM-dd HH:mm:ss' /></td>
													<td>
														<div class="tpl-table-black-operation">
															<a onclick="changeGood('${good.goodId }')"> <i class="am-icon-pencil"></i>
																编辑
															</a> <a onclick="doDel('${good.goodId }')"
								...
								<div class="am-u-lg-12 am-cf">
									...
									<div class="am-fr">
										<ul class="am-pagination tpl-pagination">
											<li><a href="good.action?page=1">«</a></li>
											<c:forEach begin="1" end="${goodList.pages }" var="n">
												<c:if test="${n==goodList.pageNum }">
													<li class="am-active"><a href="good.action?page=${n }">${n }</a></li>
												</c:if>
												<c:if test="${n!=goodList.pageNum }">
													<li><a href="good.action?page=${n }">${n }</a></li>
												</c:if>
											</c:forEach>
											<li><a href="good.action?page=${goodList.lastPage }">»</a></li>
										</ul>
									</div>
								</div>
							</div>
	...
	<script type="text/javascript">
		$(".tpl-pagination a").click(function(){
			$(this).attr("href",$(this).attr("href")+"&goodType="+$("#goodType").val()+"&goodId="+$("#goodId").val());
		});
		$("#addNew").click(function(){
			location.href="addGood.action";
		});
		function changeGood(goodId){
			location.href="addGood.action?goodId="+goodId;
		};
		function doMyQuery(){
			location.href="good.action?page=1&goodType="+$("#goodType").val()+"&goodId="+$("#goodId").val();
		}
		function doDel(goodId){
			con=confirm("是否确认删除商品"+goodId+"?");
			if(con){
				$.ajax({
					type : 'post',
					url : 'doDelGoodAjax.action',
					data : 'goodId=' + goodId,
